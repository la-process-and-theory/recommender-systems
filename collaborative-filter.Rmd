...
diffuculty <- read.csv("difficulty.csv")
```
...
library(dplyr) 
I2 <- select(interest, 2:7)
I2 <- as.matrix(I2)
rownames(I2) <- interest$name
D2 <- select(difficulty, 2:7)
D2 <- as.matrix(D2)
rownames(D2) <- diffuculty$name
```
...
```{r}
my.name <- "Ling Ai" #Input your name as it appears in the data set
