...
```{r}
my.name <- "Yawei Zhu" #Input your name as it appears in the data set
...
D2 <- distinct(D1,name,.keep_all = TRUE)
rownames(D2) <- D2$name
D2$name <- NULL
D2<-as.matrix(D2)
D.SIM <- cosine(D2)
...
I2 <- gather(I2,unit,interest,-name)

D1 <- read.csv("difficulty.csv")
D1 <- unite(D1,"name",first,last,sep=" ")
D2 <- distinct(D1,name,.keep_all = TRUE)
D2 <- gather(D2, unit, difficulty,-name)

C1 <- data.frame(D2$name, I2$unit, I2$interest, D2$difficulty)
