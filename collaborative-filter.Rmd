...
```{r}
my.name <- "Minruo Wang" #Input your name as it appears in the data set
...
# Convert interest and difficulty to long format
I1.5 <- unite(I1, "name", first, last, sep = " ") # combine names
I1.5 <- distinct(I1.5, name, .keep_all= TRUE)
I3 <- gather(I1.5, unit, interest, 2:8)

D1.5 <- unite(D1, "name", first, last, sep = " ") # combine names
D1.5 <- distinct(D1.5, name, .keep_all= TRUE)
D3 <- gather(D1.5, unit, difficulty, 2:8)
