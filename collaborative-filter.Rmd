...
```{r}
#Since I can't find my name in the matrix. Then, I will use my friend name to take the test.
my.friend.name <- "chaoxiong chen" #Input your name as it appears in the data set

head(rownames(I.SIM[order(I.SIM[my.friend.name,], decreasing = TRUE),]), n = 3) #This code orders the column of the matrix corresponding to your UNI according to similarity and returns the top two UNI ids for the students who's interests are most similar to yours
```
...
library(tidyr)
library(lsa)
T1 <- unite(D1, "name", first,last, sep = " ")#combine names
T2 <- unite(I1, "name", first,last, sep = " ")#combine names
T1<-distinct(T1,first,.keep_all=TRUE)
T2<-distinct(T2,first,.keep_all = TRUE)
I3 <- gather(T2,unit,interest, 2:7)
D3 <- gather(T1, unit, difficulty, 2:7)
...
C4 <- as.data.frame(C4)
C4 <- C4[c(-173,-71,-19,-121,-223,-269,-15,-117,-219),]
...
head(rownames(C.SIM[order(C.SIM[ne,], decreasing = TRUE),]), n = 1)
