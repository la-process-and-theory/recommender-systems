...
I2 <- select(I1, -stid) %>% as.matrix()
D2 <- select(D1, -stid) %>% as.matrix()
...
# Ideally you do not want to count NAs as zeros in the cosine similarity calculation because NAs mean that there was not an answer given. NAs do not mean that the value 0 was given.
# However, I will continue to keep the zeros in order to calculate cosine similarity matrices. 
```
...
```{r}
my.uni <- "utp2000" #Input your UNI id in here
...
diag(D.SIM) <- NA
my.uni <- "utp2000" #Input your UNI id in here
head(rownames(D.SIM[order(D.SIM[my.uni,], decreasing = TRUE),]), n = 2)
```
...
C1 <- filter(C1, difficulty > 0) #filter because 0 is NA, but NOTE: jx2196 is completely filtered out (had 0 for difficulty but not 0 for interest)
