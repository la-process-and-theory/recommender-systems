...
I2 <- data.frame(I1[,-1], row.names=I1[,1])
D2 <- data.frame(D1[,-1], row.names=D1[,1])
data.matrix(I2, rownames.force = NA)
data.matrix(D2, rownames.force = NA)
```
...
They should be zeros so that they when they are placed into the equation they don't return NA as an answer.
...
diag(D.SIM) <- NA
my.unit <- "pred.dif"
head(rownames(D.SIM[order(D.SIM[my.unit,], decreasing = TRUE),]), n = 2)
```
...
myvars <- c(pca1)
data.matrix(myvars, rownames.force = NA)
