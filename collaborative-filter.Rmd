...
```{r}
my.name <- "Jie Chen" #Input your name as it appears in the data set

head(rownames(I.SIM[order(I.SIM[my.name,], decreasing = TRUE),]), n = 5) #This code orders the column of the matrix corresponding to your UNI according to similarity and returns the top two UNI ids for the students who's interests are most similar to yours
```
...
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
my.unit <- "prediction.difficulty"
head(rownames(D.SIM[order(D.SIM[my.unit,], decreasing = TRUE),]), n = 1)
```
...
I3 <- na.omit(I1) #Omit NA data
I3 <- gather(I3, unit, interest, 3:9)
I3 <- unite(I3, "name", first, last, sep = " ") #Combine fisrt and last name
I3$unit <- gsub('\\..+','', I3$unit) #Substitute the unit name

D3 <- na.omit(D1) #Omit NA data
D3 <- gather(D3, unit, difficulty, 3:9)
D3 <- unite(D3, "name", first, last, sep = " ") #Combine first and last name
D3$unit <- gsub('\\..+','', D3$unit) #Substitute the unit name

# Merge interest and difficulty
C1 <- merge(I3, D3, by=c("name", "unit"))
...
```
For those who just complete the "prediction" unit, they can learn visualization unit next.
