...
library(dplyr)
I2<-as.matrix(select(I1, 2:7))
D2<-as.matrix(select(D1,2:7))
rownames(I2)<- I1$name
rownames(D2)<-D1$name
```
...
```{r}
my.name <- "Juye Wang" #Input your name as it appears in the data set
...
A1<-data.frame(I3$name,I3$unit,I3$interest,D3$difficulty)
names(A1)<-c("name","unit","interest","difficulty")
A1<-filter(A1,interest>0)
A2<-select(A1,"interest","difficulty")

pc <- prcomp(A2)

A3<-data.frame(A1$name, A1$unit,pc$x)
A4<-select(A3,A1.name,A1.unit,PC1)

A4$A1.unit=gsub(".int","",A4$A1.unit)

A5<-spread(A4,A1.name, PC1)
row.names(A5) <- A5$A1.unit
A5$A1.unit=NULL
A5<-ifelse(is.na(A5),0,A5)
A5<-t(A5)
