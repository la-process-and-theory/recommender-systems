...
```{r}
my.name <- "han ge" #Input your name as it appears in the data set
...
I3 <- gather(I1,unit,interest, 2:8,na.rm = TRUE)
D3 <- gather(D1, level, difficulty,2:8,na.rm = TRUE)
...
C4$C1.unit <- gsub(".int", "", C4$C1.unit)
  ...
  C5<-group_by_at(C4,vars(-PC1)) 
 C5<- mutate(C5,row_id=1:n()) 
 C5<-ungroup(C5)   # build group index
  C5<-spread(C5,key=C1.name, value=PC1)   # spread
  C5<-select(C5,-row_id)
C5<-C5[-c(2, 4, 6,8,10,12,14), ] 
C5
rownames(C5)<-c("looperest","neuralerest","nlperest","predictionerest","snaerest","sqlerest","vizerest")
C5$C1.unit <- NULL
C5 <- as.matrix(C5)
...
head(rownames(C.SIM[order(C.SIM["predictionerest",], decreasing = TRUE),]), n = 1)
