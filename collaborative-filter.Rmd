...
I3 <- data.matrix(I1[,-1])
rownames(I3) <- i1names
pred <- cbind(I3[,1],D2[,1])
nlp <- cbind(I3[,2],D2[,2])
sna <- cbind(I3[,3],D2[,3])
neural <- cbind(I3[,4],D2[,4])
viz <- cbind(I3[,5],D2[,5])
loop <- cbind(I3[,6],D2[,6])

pred_pca <- prcomp(pred)
nlp_pca <- prcomp(nlp)
sna_pca <- prcomp(sna)
neural_pca <- prcomp(neural)
viz_pca <- prcomp(viz)
loop_pca <- prcomp(loop)
...
combined_matrix <- cbind(pred_pca[["x"]][,1], 
                         nlp_pca[["x"]][,1],
                         sna_pca[["x"]][,1],
                         neural_pca[["x"]][,1],
                         viz_pca[["x"]][,1],
