...
I2<-select(d1,2:7)
I2[is.na(I2)] <- 0
#0 and NA are the same (except value from 0-5), becasue you can trying to find the similarity. you can even enter 99
I2<-as.matrix(I2)
rownames(I2)<-d1$name
D2<-select(dt,2:7)
D2[is.na(D2)]<-0
...
#The missing values should be coded with 0 since cos similarity cannot understand "NA"
I2 <- t(I2)
...
#If you enter n=-3, it gives you the last three
#[]refers places in the data frame, need to use ""
...
#gather是把d1的数据提取出来；unit和interest代表着行和列，你也可以不写，系统会自动生成
D3<-gather(dt, name,difficulty)

C1 <- cbind.data.frame(I3,D3)[,-4]
#cbind=column bind,一般combine完都是matrix，你要变成data frame；【行，列】，如果删行就【-4，】；为什么删第四列是因为重复了
...
pc <- prcomp(C2)
#把difficulty和interest结合起来-> create a composite measure from interest and difficulty

C3 <- data.frame(C1$name, C1$unit, pc$x)
C4<-select(C3, C1.name, C1.unit, PC1)
C4$C1.unit <- gsub(".int", "", C4$C1.unit)
#字符替换Remove int from unit label；如果我把C4$C1.unit换成C6那他就会变成一个新的vector；但只有一列数
C5<-spread(C4, C1.name, PC1)
#spread和gather是互逆的，长的数据变宽->宽的数据变窄
row.names(C5) <- C5$C1.unit
#把12345变成C5$C1.unit
C5$C1.unit <- NULL
#Both NA and null represents missing or undefined values; Null is undefined value, NA is not avaliable (missing value)；把那一列C1.unit删掉
C5 <- as.matrix(C5)
#变成matrix 因为cos只能接受matrix
C5 <- ifelse(is.na(C5), 0, C5)
#ifelse(test, yes, no) 如果是NA 变成0，因为cos只能run 0 不能run NA
C5 <- t(C5)
#Given a matrix or data.frame x, t returns the transpose of x. 转置
C.SIM <- cosine(C5)
diag(C.SIM) <- NA
...
#order(C.SIM["pred",], decreasing = TRUE),代表着降序排列(上面的数字最大) [1] 2 5 1 6 3 4
#rownames 找出代表的unit的名字
```
