...
difficulty_matrix = select(diff, 2:7)
interest_matrix = select(interest, 2:7)

difficulty_matrix = as.matrix(difficulty_matrix)
interest_matrix = as.matrix(interest_matrix)

rownames(difficulty_matrix) = diff$name
rownames(interest_matrix) = interest$name
...
D.SIM = cosine(difficulty_matrix)
diag(D.SIM) = NA
...
interest3 <- gather(interest,unit,interest, 2:7)
D3 = gather(diff, name, difficulty)

C1 = data.frame(interest3$name, interest3$unit, interest3$interest, D3$difficulty)
