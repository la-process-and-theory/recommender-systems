...
I1a <- subset(I1, select=c(2:7))
D1a <- subset(D1, select=c(2:7))

row.names(I1a) <- I1$stid
row.names(D1a) <- D1$stid
...
#Missing values needs to be coded as 0. The algorithm cannot process NAs.
...
```{r}
my.uni <- "an2825" #Input your UNI id in here
...
```{r}
M1<-left_join(D1, I1, by = "stid")
M1a <- subset(M1, select=c(2:13))
row.names(M1a) <- M1$stid
M2 <- scale(M1a, center = TRUE)
...
```{r}
pca$rotation
MM <- as.matrix(pca$rotation)
MM1 <- subset(MM, select=c(1:6))
loadings <- abs(pca$rotation)
sweep<-sweep(loadings, 2, colSums(loadings), "/")
```
