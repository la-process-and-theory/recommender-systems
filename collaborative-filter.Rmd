...
# We should code the missing values with zeros.
...
```{r}
my.uni <- "yz3204" #Input your UNI id in here
...
D.SIM <- cosine(MD1)
diag(D.SIM) <- NA
current.unit <- "loop.dif"
head(rownames(D.SIM[order(D.SIM[current.unit,], decreasing = TRUE),]), n = 1) 
```
