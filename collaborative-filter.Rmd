...
interest$first <- NULL
interest$last <- NULL
difficultymatrix <- data.matrix(difficulty, rownames.force = NA) 
interestmatrix <- data.matrix(interest, rownames.force = NA)
```
...
aggdatainterest <- t(interestmatrix)
aggdatadifficulty <- t(difficultymatrix)
...
library(lsa)
cosinedata <- cosine(aggdata) #This command generates the cosine similarity values as a new matrix. Click on I.SIM in the Global Environment pane to see what it looks like.
diag(cosinedata) <- NA #Since each student will be most similar to themselves we want to remove that information
```

Now, we can make a quick query to find out which students are most similar to you. 
```{r}
my.name <- "NA" #Input your name as it appears in the data set

head(rownames(cosinedata[order(cosinedata[my.name,NA], decreasing = TRUE),]), n = 2) #This code orders the column of the matrix corresponding to your UNI according to similarity and returns the top two UNI ids for the students who's interests are most similar to yours
```
