...
```{r}
my.name <- "Shuyue Zhou" #Input your name as it appears in the data set
...
D2 <- na.omit(D2) #omit rows with N/A 
rownames(D2) <- D2$name
D2$name <- NULL
D2 <- as.matrix(D2)
```
...
I9 <- unite(I1, "name", first,last, sep = " ")
I9 <- distinct(I9,name, .keep_all= TRUE)
D9 <- unite(D1, "name", first,last, sep = " ")
D9 <- distinct(D9,name, .keep_all= TRUE)

I3 <- gather(I9, unit,interest, 2:8)
D3 <- gather(D9, name, difficulty, 2:8)
