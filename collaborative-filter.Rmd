...
```{r}
inter <- read.csv("interest.csv")
diffi <- read.csv("difficulty.csv")
```
...
I2 <- select(inter, 2:7)
I2 <- as.matrix(I2)
rownames(I2) <- inter$name
D2 <- select(diffi, 2:7)
D2 <- as.matrix(D2)
rownames(D2) <- diffi$name
```
...
```{r}
my.name <- "Shuo-Yuan Kao" #Input your name as it appears in the data set
