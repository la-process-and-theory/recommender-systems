...
library(readr)
D1<- read_csv("difficulty.csv")
View(D1)
library(readr)
I1<- read_csv("interest.csv")
View(I1)
...
# I haved tried to coded all missing value as "NA", and all the data in I.SIM became NA or 1. So I think all missing values should be coded with zeros.
I.SIM <- cosine(I2)
I.SIM 
...
diag(D.SIM) <- NA
unit<-"pred.dif"
head(rownames(D.SIM[order(D.SIM[unit,], decreasing = TRUE),]), n = 1) 
```
...
D3 <- gather(D1, student, difficulty, 2:7)
