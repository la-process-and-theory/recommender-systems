...
=======
D1 <- read.csv("difficulty.csv")
>>>>>>> a69773191d1507cb1b8664503fdc8d5fc1725d09
...
```{r}
my.name <- "Ningyao Xu" #Input your name as it appears in the data set
head(rownames(I.SIM[order(I.SIM[my.name,], decreasing = TRUE),]), n = 5) #This code orders the column of the matrix corresponding to your name according to similarity and returns the top five students who's interests are most similar to yours.
```
...
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
pred.dif <- "nlp.difficulty"
head(rownames(D.SIM[order(D.SIM[pred.dif,], decreasing = TRUE),]), n = 2) 

# Therefore, the topic of neural and visualization have the most similar difficulty with nature language processing.
...
I3 <- unite(I1, "name", first,last, sep = " ")
I4 <- gather(I3,unit,interest, 2:8)

D3 <-  unite(D1, "name", first,last, sep = " ")
D4 <- gather(D3, unit, difficulty,2:8)

C1 <- data.frame(I4$name, I4$unit, I4$interest, D4$difficulty)
...
# There are several people who filled in the survey several times with different answers, so I decided to leave the last answer they entered
C4 <-C4[-c(71,19,121,266,15,217),]
C5 <- spread(C4, C1.unit, PC1)
# Some of them leave it blank, so these NAs have to be removed.
C5 <- na.omit(C5)
...
head(rownames(C.SIM[order(C.SIM["neural",], decreasing = TRUE),]), n = 1)
 ...
 #  Nature Language Processing is the most similar unit to the Neural Network.
=======
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
pred.dif <- "nlp.difficulty"
