...
row.names(I1)<-D1$stid
D1$stid<-NULL
I2 <- read.csv("interest.csv",header=TRUE)
I2$stid<-NULL
row.names(D1)<-D1$stid
row.names(I2)<-D1$stid
I3 <-read.csv("interest.csv")
row.names(I2) <- I3$stid 
```
...
library(lsa)
#Look at your data, missing values are coded with zero not NA. Investigate cosine similarity and decide whether your missing values should be coded with zeros or NAs. Explain your choice. Missing values should be coded as zeros 
as.matrix(I1)
I1<-"I2"
...
Now, we can make a quick query to find out which students are most similar to you. SZ2686!
```{r}
my.uni <- "rc3153" #Input your UNI id in here
