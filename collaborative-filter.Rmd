...
```{r}
my.name <- "Yixuan Zhu" #Input your name as it appears in the data set
...
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
pred.dif <- "sql.difficulity"
head(rownames(D.SIM[order(D.SIM[pred.dif,], decreasing = TRUE),]), n = 1) 
#Error in D.SIM[pred.if, ] : subscript out of bounds
```
