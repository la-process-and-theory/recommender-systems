...
Now create a unit-based, rather than student-based similarity matrix for difficulty. Then use your similarity matrix to provide a 
suggested next unit to a student who is looking for the unit that is most similar in terms of difficulty to the "prediction" unit. 
``` {r}
...
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
head(rownames(D.SIM[order(D.SIM[my.name], decreasing = TRUE),]), n = 1) 
```
