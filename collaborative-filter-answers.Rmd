...
```{r}
my.name <- "Yujun Zhang" #Input your name as it appears in the data set
...
I2 <- distinct(I1,name, .keep_all = TRUE)
I3 <- gather(I2,unit,interest,-name)

D1 <- read.csv("difficulty.csv")
D1 <- unite(D1,"name",first,last,sep = " ")
D2 <- distinct(D1,name,.keep_all = TRUE)
D3 <- gather(D2, unit, difficulty,-name)
