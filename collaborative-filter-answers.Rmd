...
#install.packages("lsa") #You will need to install the lsa package to access the cosine(cos) command.
library(lsa)
...
```{r}
my.name <- "Xiaojue Liu" #Input your name as it appears in the data set
...
#install.packages("lsa") #You will need to install the lsa package to access the cosine(cos) command.
library(lsa)
D.SIM2 <- cosine(D2) #This command generates the cosine similarity values as a new matrix. Click on I.SIM in the Global Environment pane to see what it looks like.
diag(D.SIM2) <- NA
...
pred.dif <- 4 #Input your name as it appears in the data set
head(rownames(D.SIM[order(I.SIM[pred.dif,], decreasing = TRUE),]), n = 3)
...
D1<-distinct(D1,first,.keep_all=TRUE)
I1<-distinct(I1,first,.keep_all = TRUE)
I3 <- gather(I1,unit,interest, 3:9)
D3 <- gather(D1, unit, difficulty,3:9)

C1 <- data.frame(I3, D3$difficulty)
C1 <- unite(C1, "name", first,last, sep = " ")

names(C1) <- c("name", "unit", "interest", "difficulty")

C1 <- filter(C1, C1$difficulty > 0)
...
head(rownames(C.SIM[order(C.SIM[1,], decreasing = TRUE),]), n = 1)
