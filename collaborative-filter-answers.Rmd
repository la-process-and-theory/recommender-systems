...
```{r}
my.name <- "Xiaowen Chen" #Input your name as it appears in the data set
...
```{r}
my.name <- "Xiaowen Chen" #Input your name as it appears in the data set
head(rownames(D.SIM[order(D.SIM[my.name,], decreasing = TRUE),]), n = 3) #This code orders the column of the matrix corresponding to your name according to similarity and returns the top two UNI ids for the students who's interests are most similar to yours
```
unit base
```{r}
D2 <- unite(D1, "name", first,last, sep = " ")#combine names
...
D2$name <- NULL
D2 <- as.matrix(D2)
D.SIM <- cosine(D2) #This command generates the cosine similarity values as a new matrix. Click on D.SIM in the Global Environment pane to see what it looks like.
D.SIM
diag(D.SIM) <- NA
...
D.SIM <- cosine(D2)
diag(D.SIM) <- NA
pre.dif <- "nlp.difficulty"
head(rownames(D.SIM[order(D.SIM[pre.dif,], decreasing = TRUE),]), n = 1) 
```
...
#use data frame to gather

I3 <- gather(I1,unit,interest,2:7)
D3 <- gather(D1,name,difficulty)

#I2 <- distinct(I1,name, .keep_all= TRUE)#remove duplicate students
#I2 <- unite(I1, "name", first,last, sep = " ")#combine names
#I2 <- as.matrix(I2)
#I3 <- gather(I2,unit,interest,2:8)
#I3[is.na(I3)]<-0

#combine names
#D2 <- distinct(D1,name, .keep_all= TRUE)#remove duplicate students
#D2 <- unite(D1, "name", first,last, sep = " ")
#D3 <- gather(D2, unit, difficulty,2:8)
#D3[is.na(D3)]<-0
#D2 <- as.matrix(D2)
#rownames(D2) <- D2$name
#D2$name <- NULL
...
library(tidyr)
library(lsa)
I3 <- gather(I1,unit,interest, 3:9)
D3 <- gather(D1, name, difficulty)
C1 <- data.frame(I3$name,I3$unit,I3$interest, D3$difficulty)
names(C1) <- c("name", "unit", "interest", "difficulty")
C1 <- filter(C1, difficulty > 0)
