...
library(dplyr) 
I2 <- select(I1, 2:7) #The select function takes the matrix, gets rid of the names, and flips it so that the names are on the rows.  
I2 <- as.matrix(I2) #Turn the data frame into a matrix
rownames(I2) <- I1$name #We assign the row names to be the names of the student
...
I2 <- t(I2) #We transpose the matrix letting units be in the rows and students be in the columns
I2[is.na(I2)] <- 0 #NA can't be understood under cosine similarity so we substitute it to zero.
D2[is.na(D2)] <- 0 
...
my.name <- "Juan Djuwadi" #Input your name as it appears in the data set
head(rownames(I.SIM[order(I.SIM[my.name,], decreasing = TRUE),]), n = 2) 

#This code orders the column of the matrix corresponding to your UNI according to similarity and returns the top two UNI ids for the students who's interests are most similar to yours. The function returns the two names that are most similar to you. 
...
D.SIM <- cosine(D2) #Find the cosine similarity for the difficulty 
diag(D.SIM) <- NA #People will be mot similar to themselves, so we set the diagonal values to NA. 
...
head(rownames(C.SIM[order(C.SIM["nlp",], decreasing = TRUE),]), n = 1)
